...
export const toggleLessonVipAPI = async (id, isVip) => {
  const { data } = await api.put(`/lessons/${id}`, { isVip });
  return data;
};

// VIP Codes API
export const fetchVipCodesAPI = async () => {
  const { data } = await api.get('/vip-codes');
  return data;
};

export const createVipCodeAPI = async (payload) => {
  const { data } = await api.post('/vip-codes', payload);
  return data;
};

export const deleteVipCodeAPI = async (id) => {
  const { data } = await api.delete(`/vip-codes/${id}`);
  return data;
};

export const redeemVipCodeAPI = async (code) => {
  const { data } = await api.post('/user/redeem-code', { code });
  return data;
};
