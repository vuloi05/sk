        ...
        h('div', { style: { display: 'flex', gap: '2rem', marginBottom: '1rem' } },
          ...
          h('div', { style: { flex: 1, display: 'flex', flexDirection: 'column', gap: '1rem' } },
             ...
             h('div', { style: { display: 'flex', alignItems: 'center', gap: '10px', marginTop: '10px' } },
               h('input', { type: 'checkbox', id: 'lesson-vip', style: { width: '20px', height: '20px', cursor: 'pointer' } }),
               h('label', { className: 'form-label', style: { marginBottom: 0, cursor: 'pointer', color: '#ffb700' }, htmlFor: 'lesson-vip' }, '⭐ Đánh dấu là bài học VIP')
             )
          )
        )
      ) : null,
...
async function handleSave() {
  ...
  const isVip = document.getElementById('lesson-vip').checked;
