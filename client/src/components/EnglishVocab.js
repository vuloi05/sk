...
function renderContent(page) {
  ...
  if (currentMode !== 'session') {
    const header = h(
      'div',
      { className: 'flex justify-between items-start mb-lg mt-xl' },
      h('div', {}, 
        h('h2', { className: 'mb-xs' }, 'Bộ 5000 Từ Vựng Tiếng Anh thông dụng (Oxford)')
      ),
...
function renderDashboard(page) {
  ...
  const btnContainer = h('div', { className: 'flex flex-col items-center justify-center mt-lg gap-sm' });

  const startBtn = h(
    'button',
    {
      className: 'btn btn-primary btn-lg',
      style: { minWidth: '300px', fontSize: '1.2rem', padding: '16px 32px', borderRadius: '50px' },
    ...
    sessionStats.totalDue > 0 ? `▶ BẮT ĐẦU HỌC (${sessionStats.totalDue} TỪ)` : '🎉 Đã hoàn thành mục tiêu hôm nay!'
  );
...
function renderSession(page) {
  const container = h('div', { className: 'flex flex-col items-center animate-slide-up w-full mt-xl' });
  
  const total = sessionQueue.length;
  const idx = sessionIndex;

  container.appendChild(
    h('div', { className: 'practice-header', style: { width: '100%', maxWidth: '800px', justifyContent: 'flex-start', gap: '16px', marginBottom: '24px' } },
      h('button', {
      ...
      idx < total ? h('span', { className: 'practice-progress-text' }, `Thẻ ${idx + 1} / ${total}`) : null
    )
  );
