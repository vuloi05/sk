    ...
    h('div', { className: 'header-inner' },
      ...
      h('nav', { className: 'header-menu' },
        h('ul', {},
          ...
          (store.get('currentUser')?.role === 'admin') ? h('li', { className: (store.get('route') === ROUTES.ADMIN_PANEL || store.get('route') === ROUTES.UPLOAD) ? 'active' : '' },
            h('a', {
              href: '#',
              onClick: (e) => { e.preventDefault(); store.set('route', ROUTES.ADMIN_PANEL); }
            }, 'Trang Quản trị')
          ) : null,

          // Từ vựng & Cài đặt (User Only)
          (store.get('currentUser')?.role !== 'admin') ? h('li', { className: (store.get('route') === ROUTES.VOCAB_EN || store.get('route') === ROUTES.VOCABULARY) ? 'active dropdown-parent' : 'dropdown-parent' },
            h('a', {
              href: '#',
              onClick: (e) => { e.preventDefault(); }
            }, 'Từ vựng ▾'),
            h('ul', { className: 'dropdown header-dropdown' },
              h('li', {},
                h('a', {
                  href: '#',
                  onClick: (e) => { e.preventDefault(); store.set('route', ROUTES.VOCAB_EN); }
                }, 'Tiếng Anh')
              ),
              h('li', {},
                h('a', {
          ...
          (store.get('currentUser')?.role !== 'admin') ? h('li', { className: store.get('route') === ROUTES.SETTINGS ? 'active' : '' },
            h('a', {
              href: '#',
